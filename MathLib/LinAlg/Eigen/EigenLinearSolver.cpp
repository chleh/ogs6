...
void EigenLinearSolver::setOption(BaseLib::ConfigTree const& option)
{
    ...
    //! \ogs_project_file_parameter{linear_solver__eigen}
    auto const ptSolver = option.getConfSubtreeOptional("eigen");
    if (!ptSolver)
        return;

    //! \ogs_project_file_parameter{linear_solver__eigen__solver_type}
    ...
    //! \ogs_project_file_parameter{linear_solver__eigen__precon_type}
    ...
    //! \ogs_project_file_parameter{linear_solver__eigen__error_tolerance}
    if (auto error_tolerance = ptSolver->getConfParamOptional<double>("error_tolerance")) {
        _option.error_tolerance = *error_tolerance;
    }
    //! \ogs_project_file_parameter{linear_solver__eigen__max_iteration_step}
