...
#include "ProcessLib/IncompressibleStokesBrinkmanModified/CreateIncompressibleStokesBrinkmanModifiedProcess.h"
...
// #include
// "ProcessLib/ThermalTwoPhaseFlowWithPP/CreateThermalTwoPhaseFlowWithPPProcess.h"
    ...
    for (auto process_config : processes_config.getConfigSubtreeList("process"))
    {
        ...
        else if (type == "TCHS_STOKES")
        {
            ...
            switch (process_config.getConfigParameter<int>("dimension"))
            {
                case 2:
                    process =
                        ProcessLib::TCHSStokes::createTCHSStokesProcess<2>(
                            *_mesh_vec[0], std::move(jacobian_assembler),
                            _process_variables, _parameters, integration_order,
                            process_config);
                    break;
// TODO saving some more compile time.
...
#endif
            if (type == "INCOMPRESSIBLE_STOKES_BRINKMAN")
        {
        ...
        else if (type == "INCOMPRESSIBLE_STOKES_BRINKMAN_MODIFIED")
        {
            //! \ogs_file_param{prj__processes__process__INCOMPRESSIBLE_STOKES_BRINKMAN_MODIFIED__dimension}
            switch (process_config.getConfigParameter<int>("dimension"))
            {
                case 2:
                    process = ProcessLib::IncompressibleStokesBrinkmanModified::
                        createIncompressibleStokesBrinkmanModifiedProcess<2>(
                            *_mesh_vec[0], std::move(jacobian_assembler),
                            _process_variables, _parameters, integration_order,
                            process_config);
                    break;
                case 3:
                    process = ProcessLib::IncompressibleStokesBrinkmanModified::
                        createIncompressibleStokesBrinkmanModifiedProcess<3>(
                            *_mesh_vec[0], std::move(jacobian_assembler),
                            _process_variables, _parameters, integration_order,
                            process_config);
                    break;
                default:
                    OGS_FATAL(
                        "INCOMPRESSIBLE_STOKES_BRINKMAN_MODIFIED process does "
                        "not support given dimension");
            }
        }
        else
#if 0
