...
#include "ProcessLib/IncompressibleStokesBrinkman/CreateLocalAssemblers.h"
...
IncompressibleStokesBrinkmanProcess<DisplacementDim>::
    IncompressibleStokesBrinkmanProcess(
        MeshLib::Mesh& mesh,
...
template <int DisplacementDim>
bool IncompressibleStokesBrinkmanProcess<DisplacementDim>::isLinear() const
{
    return false;
}
...
IncompressibleStokesBrinkmanProcess<DisplacementDim>::getMatrixSpecifications(
...
template <int DisplacementDim>
void IncompressibleStokesBrinkmanProcess<DisplacementDim>::constructDofTable()
{
...
template <int DisplacementDim>
void IncompressibleStokesBrinkmanProcess<DisplacementDim>::
                              ...
                              unsigned const integration_order)
{
    ...
    ProcessLib::IncompressibleStokesBrinkman::createLocalAssemblers<
        DisplacementDim, IncompressibleStokesBrinkmanLocalAssembler>(
...
template <int DisplacementDim>
void IncompressibleStokesBrinkmanProcess<
...
void IncompressibleStokesBrinkmanProcess<
    DisplacementDim>::assembleConcreteProcess(const double t,
                                              ...
                                              GlobalVector& b)
{
    DBUG("Assemble the equations for IncompressibleStokesBrinkman");
                                        ...
                                        GlobalMatrix& Jac)
{
    ...
    {
        DBUG(
            "Assemble the Jacobian of IncompressibleStokesBrinkman for the "
            "monolithic"
            " scheme.");
    ...
    else
    {
        ...
        if (_coupled_solutions->process_id == 0)
        {
            DBUG(
                ...
                "IncompressibleStokesBrinkman for the staggered scheme.");
        }
        else
        {
            DBUG(
...
{
    DBUG("PreTimestep IncompressibleStokesBrinkmanProcess.");
...
void IncompressibleStokesBrinkmanProcess<
    DisplacementDim>::postTimestepConcreteProcess(GlobalVector const& x,
                                                  const int process_id)
{
    DBUG("PostTimestep IncompressibleStokesBrinkmanProcess.");
    GlobalExecutor::executeMemberOnDereferenced(
                                                         ...
                                                         const int process_id)
{
    ...
    DBUG("PostNonLinearSolver IncompressibleStokesBrinkmanProcess.");
...
IncompressibleStokesBrinkmanProcess<DisplacementDim>::getDOFTable(
    const int process_id) const
{
    if (hasMechanicalProcess(process_id))
    {
