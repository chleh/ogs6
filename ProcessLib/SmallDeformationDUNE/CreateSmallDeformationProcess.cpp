    ...
    BaseLib::ConfigTree const& config)
{
    ...
    config.checkConfigParameter("type", "SMALL_DEFORMATION_DUNE");
    DBUG("Create SmallDeformationProcess (DUNE).");
    ...
    auto* grid = dynamic_cast<MeshLib::DUNEMesh<DisplacementDim>*>(&mesh);
    if (!grid)
    {
        OGS_FATAL("Mesh is not a DUNE mesh!");
    }

    SmallDeformationProcessData<DisplacementDim> process_data{
        std::move(material), solid_density, specific_body_force,
        reference_temperature, *grid};
