 ...
 */
double getImPartOfPermittivityDebye(const double diff_eps, const double omega, const double t_relax)
{
...
double getImPartOfPermittivityHF_II(const double T)
{
    ...
    const double imPart_II_HF  = getImPartOfPermittivityDebye(diff_eps_II_HF, omega, t_relax_II_HF);
...
double getImPartOfPermittivityHF_III(const double T, const double loading)
{
    ...
    const double t_relax_III_HF = t_relax0_III_HF * std::exp(get_EA_III_HF(loading) / kB / T);
    const double imPart_III_HF  = getImPartOfPermittivityDebye(diff_eps_III_HF, omega, t_relax_III_HF);
...
double getImPartOfPermittivityHF(const double T, const double loading)
{
    ...
    return
        w       * getImPartOfPermittivityHF_II (T) +
...
{
    return getImPartOfPermittivityHF(T, loading)
            + getImPartOfPermittivityZ(T)
...
{
    // Note: The term m_ZS/rho_ZS is not needed here, sinve we return a volumetric value!
    return getZ(loading) * getImPartOfPermittivity(T, loading)
            * 4 / pi / d_inner / d_inner;
}
