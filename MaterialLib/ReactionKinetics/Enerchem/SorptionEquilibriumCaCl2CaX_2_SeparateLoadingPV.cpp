...
static double CaCl2_p_V_over_salt_solution(const double molality,
                                           const double T)
{
    const double a0 = 46671.1524;
    const double a1 = -114.854874;
    const double a2 = -0.13948502;

    const double b0 = 2.4975e+06;
    const double b1 = -17368.1039;
    const double b2 = 30.2577421;

    const double c = -2.49597774;
    const double d = 7.19440353;

    const double p_V_eq =
        a0 + a1 * T + a2 * T * T +
        (b0 + b1 * T + b2 * T * T) / (1.0 + std::exp((molality - c) / d));
    return p_V_eq;
}
                                                ...
                                                const double salt_loading)
{
    ...
    double s_i, p_V_eq_salt_solution;
    ...
    {  // deliquescence
        ...
        {
            p_V_eq_salt_solution =
                CaCl2_p_V_over_salt_solution(molality, T_Ads);
        }
    }
    ...
    return {C_eq_Z, p_V_eq_salt_solution};
}
...
{
void SorptionEquilibriumCaCl2CaX_2_SeparateLoadingPV::getEquilibrium(
    const double p_Ads,
    const double T_Ads,
    ReactiveSolidState const& solid_state,
    ReactiveSolidState& equilibrium) const
{
    auto const s_ni = 0.0;  // TODO
    auto const C_eq_salt_solution = solid_state.conversion()[1];
    auto const molality = s_ni /
                          CaCl2_CaX_get_molar_mass_composite(_salt_loading) /
                          C_eq_salt_solution;
    equilibrium.conversion() =
        CaCl2_CaX_get_equilibrium(p_Ads, T_Ads, molality, _salt_loading);
}

HeatOfReactionData
SorptionEquilibriumCaCl2CaX_2_SeparateLoadingPV::getHeatOfReaction(
    const double p_Ads,
    const double T_Ads,
...
SorptionEquilibriumCaCl2CaX_2_SeparateLoadingPV::getNamedFunctions() const
{
    ...
    BaseLib::ConfigTree const& config)
{
    ...
    return std::unique_ptr<SorptionEquilibriumCaCl2CaX_2_SeparateLoadingPV>{
        new SorptionEquilibriumCaCl2CaX_2_SeparateLoadingPV{salt_loading}};
}
