...
static const MathLib::PiecewiseLinearInterpolation CaCl2_deliquescence_line(
    {// deliquescence line, T
     ...
     3.163399999999999750e+02,  3.1748689999999999663e+02,
...
{
double SorptionEquilibriumCaCl2CaX_1_Combined::getEquilibriumDensity(
    const double p_Ads, const double T_Ads)
{
    return _rho_SR_dry * (1.0 + getEquilibriumLoading(p_Ads, T_Ads));
}

double SorptionEquilibriumCaCl2CaX_1_Combined::getEquilibriumLoading(
    const double p_Ads, const double T_Ads) const
{
    ...
    {  // no deliquescence
        ...
        else
        {
            ...
            s_i = s_i_max;
            auto const s_ni = _salt_loading - s_i_max;
    ...
    {  // deliquescence
        ...
        {
            auto const s_ni = _salt_loading - s_i;
            Delta_w_ni = CaCl2_loading_not_included_salt(s_ni, p_Ads, T_Ads);
        }
    }

    const double M_Comp = CaCl2_CaX_get_molar_mass_composite(_salt_loading);
...
double SorptionEquilibriumCaCl2CaX_1_Combined::getDEquilibriumDensityDp(
    const double p, const double T)
{
    ...
    auto const pp = p * (1. + e);
    auto const pm = p * (1. - e);
    auto const rho_p = getEquilibriumDensity(pp, T);
    auto const rho_m = getEquilibriumDensity(pm, T);

    return (rho_p - rho_m) / (pp - pm);
}

HeatOfReactionData SorptionEquilibriumCaCl2CaX_1_Combined::getHeatOfReaction(
    const double p_Ads,
    const double T_Ads,
    const ReactiveSolidState* const state) const
{
    ...
    (void)T_Ads;
    (void)state;
    return Eigen::VectorXd::Constant(1, 3.6e6);  // J/kg
}

std::vector<NumLib::NamedFunction>
SorptionEquilibriumCaCl2CaX_1_Combined::getNamedFunctions() const
{
    return {{"equilibrium_loading",
             {"adsorptive_partial_pressure", "adsorbent_temperature"},
             BaseLib::easyBind(
                 &SorptionEquilibriumCaCl2CaX_1_Combined::getEquilibriumLoading,
                 *this)},
            {"loading",
             {"adsorbent_density"},
             BaseLib::easyBind([this](double rho) -> double {
                 return rho / _rho_SR_dry - 1.0;
             })},
            {"heat_of_reaction",
             {"adsorbent_temperature", "adsorbent_density"},
             BaseLib::easyBind([](double, double) -> double {
                 // TODO update
                 // return getHeatOfReaction(0.0, 0.0, 0.0);
                 return 3.6e6;  // J/kg
             })}};
}
...
createSorptionEquilibriumCaCl2CaX_1_Combined(BaseLib::ConfigTree const& config)
{
    config.checkConfigParameter("type",
                                "SorptionEquilibriumCaCl2CaX_1_Combined");
    ...
    return std::unique_ptr<SorptionEquilibriumCaCl2CaX_1_Combined>(
        new SorptionEquilibriumCaCl2CaX_1_Combined(rho_SR_dry, salt_loading));
}
