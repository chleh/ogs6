...
for t, fn in zip(ts, fns):
    ...
    if True:
        ...
        def strain_triangle_axi(cell, point_data, strain_data):
            ...
            a, b, c, d = T.flat
            T_inv = np.matrix([d, -b, -c, a]).reshape(2,2) / (a*d - b*c)
            assert np.all(T * T_inv - np.matrix([1,0,0,1]).reshape(2,2) < 1e-14)
            ...
            for node in range(3):
                ...
                try:
                    tol = 1e-8
                    assert -tol < l1 and 1 + tol > l1 \
                            and -tol < l2 and 1 + tol > l2
                except:
